...
function actualizarActual(ciudad) {
  fetch("http://localhost:5000/api/clima/actual/" + ciudad)
...
function actualizarPronostico(lat, lon) {
  fetch("http://localhost:5000/api/clima/pronostico/" + lat + "&" + lon)
    .then((Response) => Response.json())
    .then((data) => {
...
async function llamadasHistorico(lat, lon) {
  for (i = 1; i <= 5; i++) {
    var respuesta = await fetch(
      "http://localhost:5000/api/clima/historico/" +
        lat +
        "&" +
        lon +
        "&" +
  ...
  for (x in dato) {
    ...
    li.appendChild(
      document.createTextNode(
        obtenerFecha(dato[x].dt) +
          "-" +
          "Temp: " +
          dato[x].temp +
          " °C " +
          "- Viento: " +
          velocidadViento(dato[x].wind_speed) +
          " Km/h - " +
          "Humedad: " +
          dato[x].humidity +
          " % - " +
          dato[x].weather[0].description
      )
    );
    ul.appendChild(li);
  }
...
function refrescarPanelPrincipal(data) {
  ...
  document.getElementById("fechaActual").innerHTML = `Hoy ${obtenerFecha(
    data.dt
  )}`;
  ...
  document.getElementById(
    "temperaturaMaxima"
  ).innerHTML = `Máx: ${data.main.temp_max} °C`;
  document.getElementById(
    "temperaturaMinima"
  ).innerHTML = `Mín: ${data.main.temp_min} °C`;
  document.getElementById(
    "sensacionTermicaActual"
  ).innerHTML = `Sensación térmica ${data.main.feels_like} °C`;
  //Panel viento
  document.getElementById("vectorViento").innerHTML = `${velocidadViento(
    data.wind.speed
  )} km/H ${vectorViento(data.wind.deg)}`;
  document.getElementById("iconoViento").src = `iconos/wind/${vectorViento(
    data.wind.deg
  )}.png`;
  //Panel Humedad
  document.getElementById(
    "humedadActual"
  ).innerHTML = `Humedad: ${data.main.humidity} %`;
  document.getElementById(
    "presionActual"
  ).innerHTML = `Presión: ${data.main.pressure} hPa`;
  //Panel Salida Sol-Puesta Sol
  document.getElementById("tiempoSunrise").innerHTML = `${obtenerHora(
    data.sys.sunrise
  )}`;
  document.getElementById("tiempoSunset").innerHTML = `${obtenerHora(
    data.sys.sunset
  )}`;
}
...
function refrescarPanelPronostico() {
  ...
  for (x in pronostico) {
    ...
    btn.onclick = function () {
      refrescarDatosPronostico(this.value);
    };
...
function refrescarDatosPronostico(fecha) {
  ...
  for (x in pronostico) {
    if (obtenerFechaSinAnio(pronostico[x].dt) == fecha) {
      document.getElementById(
        "logoPronostico"
      ).src = `http://openweathermap.org/img/wn/${pronostico[x].weather[0].icon}@2x.png`;
      document.getElementById(
        "descripcionPronostico"
      ).innerHTML = `${pronostico[x].weather[0].description}`;
      document.getElementById(
        "tempPronostico"
      ).innerHTML = `Temperatura: ${pronostico[x].temp.day}`;
      document.getElementById(
        "vientoPronostico"
      ).innerHTML = `Viento: ${velocidadViento(
        pronostico[x].wind_speed
      )} ${vectorViento(pronostico[x].wind_deg)}`;
      document.getElementById(
        "humedadPronostico"
      ).innerHTML = `Humedad: ${pronostico[x].humidity}%`;
    }
  }
}
